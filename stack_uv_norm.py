# Script to stack pointings for a given list of coordinates - Flux normalized
#
...
flux = [2.15339431866, 2.24738549193, 2.11097095919]
		...
		try:
			...
			if hdr['CRVAL1'] - abs(hdr['CDELT1'] * (hdr['NAXIS1'] / 2)) < ra[i] and ra[i] < hdr['CRVAL1'] + abs(hdr['CDELT1'] * (hdr['NAXIS1'] / 2)) and hdr['CRVAL2'] - abs(hdr['CDELT2'] * (hdr['NAXIS2'] / 2)) < dec[i] and dec[i] < hdr['CRVAL2'] + abs(hdr['CDELT2'] * (hdr['NAXIS2'] / 2)):
				...
				if img_val >= 0.0:
					...
					weight_scl = [1 / flux[i]] + weight_scl
...
concat(vis = vis_concat, concatvis = myvisbase + output + '.ms', freqtol = '500MHz', dirtol = '5arcsec', copypointing = False, visweightscale = weight_scl)
